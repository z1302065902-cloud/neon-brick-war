...
export function createBrickFigure(palette: BrickPalette, opts: BrickFigureOpts = {}): THREE.Group {
  ...
  stud.position.y = RIG.headY + 0.27;
  ...
  for (const side of [-1, 1] as const) {
    ...
    pivot.position.set(side * 0.44, RIG.shoulderY, 0);
    body.add(pivot);
  ...
  gun.name = 'Gun';
  gun.position.set(0.28, -0.23, -0.05);
  ...
  for (const side of [-1, 1] as const) {
    ...
    pivot.position.set(side * 0.17, RIG.hipY, 0);
    body.add(pivot);
  ...
  if (opts.boss) {
    ...
    crown.position.y = RIG.headY + 0.4;
    body.add(crown);
